...
public class VideoAdapter extends RecyclerView.Adapter<VideoAdapter.ViewHolder>
    ...
    public VideoAdapter(Context ctx, List<Video> videos) {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, final int position) {
            holder.title.setText(allVideos.get(position).getVideo_title());
            Picasso.get().load(allVideos.get(position).getVideo_thumbnail_s()).into(holder.videoImage);

            holder.vv.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Bundle b = new Bundle();
                    b.putSerializable("video_url",allVideos.get(position));
